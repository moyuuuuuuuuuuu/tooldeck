...
RUN apt-get -o Acquire::Retries=3 update && apt-get -o Acquire::Retries=3 install -y --no-install-recommends socat ca-certificates build-essential python3 && rm -rf /var/lib/apt/lists/*
