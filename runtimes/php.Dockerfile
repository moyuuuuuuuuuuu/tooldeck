...
RUN if grep -q '^VERSION_CODENAME=bullseye$' /etc/os-release; then find /etc/apt -type f \( -name '*.list' -o -name '*.sources' \) -exec sed -i 's|https://security.debian.org/debian-security|https://snapshot.debian.org/archive/debian-security/20260831T194148Z|g' {} +; printf 'Acquire::Check-Valid-Until "false";\n' > /etc/apt/apt.conf.d/99snapshot; fi
