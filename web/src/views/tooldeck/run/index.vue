<template>
 <div class="run-page" v-loading="loading">
  <template v-if="tool">
   ...
   <ElDrawer v-model="detailOpen" :title="tool.manifest.title||tool.manifest.name" size="min(680px, 94vw)" destroy-on-close><ElTabs v-model="detailTab"><ElTabPane v-if="user.isLogin&&tool.manifest.env?.length" label="运行环境变量" name="env"><ElAlert :title="owner?'个人配置与共享给使用者的配置分别保存；其他用户的个人值不会向作者或管理员展示。':'这里保存的是你在此工具中的个人配置，与作者、管理员及其他用户完全隔离；保存后网页运行和你的 API 调用都会自动使用。'" type="warning" :closable="false" style="margin-bottom:18px"/><ToolEnvironment :tool-id="tool.id"/></ElTabPane><ElTabPane v-if="user.isLogin&&tool.api_enabled!==false" label="API 参数" name="api"><div class="api-title"><div><span>POST</span><h3>执行此工具</h3></div><ElButton text type="primary" @click="router.push('/tooldeck/api-guide')">查看通用 API 文档 →</ElButton></div><label class="endpoint-label">请求地址</label><div class="endpoint"><code>{{apiEndpoint}}</code><ElButton type="primary" plain @click="copyEndpoint">复制地址</ElButton></div><h3>input 参数</h3><ElTable :data="inputParameters" size="small"><ElTableColumn prop="name" label="参数" min-width="130"/><ElTableColumn prop="type" label="类型" width="105"/><ElTableColumn label="必填" width="65"><template #default="{row}">{{row.required?'是':'否'}}</template></ElTableColumn><ElTableColumn prop="description" label="说明" min-width="180"/></ElTable><template v-if="tool.manifest.env?.length"><h3>可选 env 临时覆盖</h3><ElTable :data="tool.manifest.env" size="small"><ElTableColumn prop="name" label="变量" min-width="150"/><ElTableColumn label="必填" width="65"><template #default="{row}">{{row.required?'是':'否'}}</template></ElTableColumn><ElTableColumn prop="description" label="说明" min-width="180"/></ElTable><p class="api-hint">env 可省略并使用已保存的个人配置；传入时仅覆盖本次运行。</p></template><h3>请求体示例</h3><pre>{{requestBodyExample}}</pre></ElTabPane></ElTabs></ElDrawer>
  </template>
...
const inputParameters=computed(()=>orderedFields.value.map(([name,field])=>({name,type:field.type+(field.format?` · ${field.format}`:''),required:tool.value?.manifest.input_schema.required?.includes(name),description:field.description||field.title||'—'})))
...
function openDetail(tab:'env'|'api'){detailTab.value=tab;detailOpen.value=true}
