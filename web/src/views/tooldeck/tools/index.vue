 ...
 <div class="tool-page">
  ...
  <div v-loading="loading" class="cards">
   <ElCard v-for="tool in filtered" :key="tool.id" shadow="never" class="tool-card">
    ...
    <div class="card-bottom"><span class="version">v{{ tool.manifest.version }}</span><ElButton type="primary" plain @click="router.push('/tooldeck/run/'+encodeURIComponent(tool.id))">运行工具 <span class="run-arrow" aria-hidden="true">→</span></ElButton></div>
   </ElCard>
  </div>
  <ElDialog v-model="uploadOpen" title="上传工具包草稿" width="520px" @closed="resetUploadForm">
   ...
   <ElFormItem label="构建环境版本"><ElSelect v-model="buildVersion" placeholder="使用代码包声明或平台默认版本" clearable style="width:100%" @change="markUploadDirty('buildVersion')"><ElOptionGroup v-for="(versions,language) in versionOptions" :key="language" :label="String(language)"><ElOption v-for="v in versions" :key="language+v" :value="String(language)+':'+v" :label="language+' '+v"/></ElOptionGroup></ElSelect></ElFormItem>
   ...
   <ElFormItem label="SSE 流式输出"><ElSelect v-model="streamMode" style="width:100%" @change="markUploadDirty('streamMode')"><ElOption value="" label="跟随工具包声明（未声明则关闭）"/><ElOption value="true" label="开启 SSE 流式输出"/><ElOption value="false" label="关闭 SSE 流式输出"/></ElSelect></ElFormItem>
...
const tools=ref<Tool[]>([]),loading=ref(false),search=ref(''),runtime=ref('')
...
type UploadManifest={schema_version?:number;name?:string;title?:string;version?:string;runtime?:string;runtime_version?:string;build_command?:string;execution?:{mode?:string;stream?:boolean};network?:{enabled?:boolean;allowed_hosts?:string[]}}
const uploadOpen=ref(false),uploadMode=ref(''),zip=ref<File>(),uploadBusy=ref(false),manifestLoading=ref(false),manifestError=ref(''),manifestSummary=ref('')
...
function markUploadDirty(field:UploadField){uploadDirty.add(field)}
function setFromManifest(field:UploadField,value:unknown){if(uploadDirty.has(field))return;if(field==='buildVersion')buildVersion.value=String(value??'');else if(field==='buildCommand')buildCommand.value=String(value??'');else if(field==='thirdParty')thirdParty.value=Boolean(value);else if(field==='allowedHosts')allowedHosts.value=String(value??'');else if(field==='streamMode')streamMode.value=String(value??'');else uploadMode.value=String(value??'')}
async function onZipChange(file:any){zip.value=file.raw;manifestError.value='';manifestSummary.value='';if(!zip.value)return;manifestLoading.value=true;try{const archive=await JSZip.loadAsync(zip.value);const entry=archive.file('tooldeck.json');if(!entry)throw new Error('ZIP 根目录缺少 tooldeck.json');const manifest=JSON.parse(await entry.async('string')) as UploadManifest;if(!manifest||typeof manifest!=='object'||manifest.schema_version!==1)throw new Error('tooldeck.json 无效或 schema_version 不是 1');const runtime={js:'node',py:'python',golang:'go'}[manifest.runtime||'']||manifest.runtime||'';if(manifest.runtime_version)setFromManifest('buildVersion',`${runtime}:${manifest.runtime_version}`);else setFromManifest('buildVersion','');setFromManifest('buildCommand',manifest.build_command||'');if(manifest.network){setFromManifest('thirdParty',manifest.network.enabled===true);setFromManifest('allowedHosts',Array.isArray(manifest.network.allowed_hosts)?manifest.network.allowed_hosts.join(', '):'')}setFromManifest('streamMode',typeof manifest.execution?.stream==='boolean'?String(manifest.execution.stream):'');setFromManifest('uploadMode',['sync','async'].includes(manifest.execution?.mode||'')?manifest.execution?.mode:'');manifestSummary.value=`已读取：${manifest.title||manifest.name||'未命名工具'} ${manifest.version?'v'+manifest.version:''}`.trim()}catch(error){manifestError.value=error instanceof Error?error.message:'无法读取 tooldeck.json'}finally{manifestLoading.value=false}}
function onZipRemove(){zip.value=undefined;manifestError.value='';manifestSummary.value=''}
function resetUploadForm(){uploadDirty.clear();buildVersion.value='';buildCommand.value='';thirdParty.value=false;allowedHosts.value='';streamMode.value='';uploadMode.value='';isPublic.value=true;notifyResult.value=false;apiEnabled.value=true;zip.value=undefined;manifestError.value='';manifestSummary.value='';uploadRef.value?.clearFiles()}
async function publish(){if(!zip.value)return;uploadBusy.value=true;try{const options:Record<string,string>={public:String(isPublic.value),notify_result:String(notifyResult.value),api_enabled:String(apiEnabled.value)};let mode='';if(uploadDirty.has('uploadMode'))mode=uploadMode.value;if(uploadDirty.has('buildVersion')){options.build_runtime=buildVersion.value.split(':')[0]||'';options.runtime_version=buildVersion.value.split(':')[1]||''}if(uploadDirty.has('buildCommand'))options.build_command=buildCommand.value;if(uploadDirty.has('streamMode')&&streamMode.value!=='')options.stream=streamMode.value;if(uploadDirty.has('thirdParty'))options.third_party=String(thirdParty.value);if(uploadDirty.has('allowedHosts'))options.allowed_hosts=allowedHosts.value;await td.upload(zip.value,'tools',mode,options);ElMessage.success('源码草稿已保存，请开始构建');uploadOpen.value=false;await router.push('/tooldeck/my-tools')}finally{uploadBusy.value=false}}
onMounted(async()=>{await load();if(useUserStore().isLogin && route.query.upload==='1')uploadOpen.value=true;if(route.query.tool)await router.replace('/tooldeck/run/'+encodeURIComponent(String(route.query.tool)))})
</script>
<style scoped>
...
.package-upload{width:100%;margin-bottom:20px}.package-upload :deep(.el-upload),.package-upload :deep(.el-upload-dragger){width:100%}
</style>
