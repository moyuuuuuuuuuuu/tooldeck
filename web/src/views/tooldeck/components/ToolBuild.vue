<template><section><p>构建固定使用所选语言版本。依赖安装和构建不会注入业务环境变量。</p><ElTag>{{ labels[tool?.build_status||'legacy'] }}</ElTag><span v-if="tool" style="margin-left:12px">{{ tool.manifest.runtime }} {{ tool.manifest.runtime_version || '旧运行环境' }}</span><ElButton style="margin-left:12px" @click="load">刷新</ElButton><ElButton v-if="tool?.build_status==='failed'" type="primary" :loading="retrying" @click="retry">重新构建</ElButton><ElAlert v-if="tool?.build_error" :title="tool.build_error" type="error" :closable="false" style="margin-top:16px"/><pre>{{ tool?.build_log || '暂无日志；依赖安装结束后将显示构建日志。' }}</pre><p v-if="tool?.build_status==='ready'">产物已保存，运行时直接复用。发布后的版本不重新构建；代码或依赖变更请上传新版本。</p></section></template>
...
const labels:Record<string,string>={legacy:'旧版直接执行',queued:'等待构建',building:'正在构建',ready:'构建成功',failed:'构建失败'}
