<template>
  <section>
    <header class="heading tooldeck-page-hero"><div><span class="page-eyebrow">TOOL MANAGEMENT</span><h1>我的工具</h1><p>管理每个版本的构建、发布状态与审核反馈。</p></div><ElButton type="primary" @click="router.push('/tooldeck/tools?upload=1')">上传工具包</ElButton></header>
    ...
    <ElTable v-loading="loading" :data="filtered" empty-text="暂无符合条件的工具">
      ...
      <ElTableColumn label="操作" min-width="300"><template #default="{row}"><ElButton link type="primary" @click="router.push('/tooldeck/run/'+encodeURIComponent(row.id))">详情</ElButton><ElButton v-if="['queued','building'].includes(row.build_status||'')" link type="primary" @click="openLog(row as Tool)">构建日志</ElButton><ElButton v-if="['pending','failed'].includes(row.build_status||'')" link type="primary" :loading="busy===row.id" @click="build(row as Tool)">{{row.build_status==='failed'?'重新构建':'开始构建'}}</ElButton><ElButton v-if="['draft','rejected'].includes(row.review_status||'')||row.withdrawn" link type="success" :disabled="busy===row.id || row.build_status!=='ready'" @click="change(row as Tool,'submit')">{{row.public===false?'发布':row.review_status==='rejected'?'重新提交':'提交审核'}}</ElButton><ElButton v-if="!row.withdrawn&&['pending','approved'].includes(row.review_status||'')" link type="danger" :disabled="busy===row.id" @click="change(row as Tool,'withdraw')">下架</ElButton></template></ElTableColumn>
    </ElTable>
    <ElDialog v-model="logOpen" :title="`${logTool?.manifest.title || '工具'} · 构建日志`" width="min(860px, 95vw)" destroy-on-close>
...
const router=useRouter(),tools=ref<Tool[]>([]),search=ref(''),filter=ref(''),loading=ref(false),busy=ref(''),logOpen=ref(false),logTool=ref<Tool>()
...
function openLog(t:Tool){logTool.value=t;logOpen.value=true}
function updateTool(t:Tool){logTool.value=t;const index=tools.value.findIndex(item=>item.id===t.id);if(index>=0)tools.value[index]=t}
onMounted(load)
</script>
<style scoped>
