<template>
  <section>
    ...
    <ElTable v-loading="loading" :data="filtered" empty-text="暂无符合条件的工具">
      ...
      <ElTableColumn label="操作" width="205"><template #default="{row}"><ElButton link type="primary" @click="router.push('/tooldeck/tools?tool='+encodeURIComponent(row.id))">详情</ElButton><ElButton v-if="!row.withdrawn" link type="danger" :disabled="busy===row.id" @click="change(row as Tool,'withdraw')">下架</ElButton><ElButton v-if="row.withdrawn || row.review_status==='rejected'" link type="primary" :disabled="busy===row.id || !['','ready'].includes(row.build_status||'')" @click="change(row as Tool,'resubmit')">{{row.withdrawn?'重新上架':'重新提交'}}</ElButton></template></ElTableColumn>
    </ElTable>
    <p class="footnote">下架仅影响所选版本，保留代码、配置和历史记录。公开版本重新提交时，将按站点当前规则审核；私有工具无需审核。</p>
  </section>
</template>
...
const labels:Record<string,string>={pending:'审核中',rejected:'已驳回',approved:'已通过',withdrawn:'已下架'}
const buildLabels:Record<string,string>={'':'可运行',ready:'构建成功',failed:'构建失败',queued:'等待构建',building:'正在构建'}
...
async function change(t:Tool,action:string){
 try{await ElMessageBox.confirm(action==='withdraw'?`下架「${t.manifest.title}」v${t.manifest.version}？其他用户将无法发起新的调用，已开始的任务会继续执行。`:'重新提交此版本？公开工具可能需要再次审核。','确认发布操作',{type:'warning',confirmButtonText:'确认',cancelButtonText:'取消'})}catch{return}
