...
export interface Tool {
 ...
 public?:boolean; review_status?:string;review_note?:string;api_enabled?: boolean; notify_result?:boolean; owner?:string
  id: string
  manifest: { runtime_version?:string;name: string; title: string; version: string; description: string; runtime: string
    execution: {mode: string; timeout_seconds: number; memory_mb: number}
...
export const td = {
  tools: () => request.get<Tool[]>({url: '/v1/tools'}),
  runs: () => request.get<Run[]>({url: '/v1/runs?mine=1'}),
  run: (id: string) => request.get<Run>({url: `/v1/runs/${id}`}),
  execute: (id: string, input: any) => request.post<Run>({url: `/v1/tools/${id}/runs`, data: {input}, timeout: 25000, headers: {'Idempotency-Key': crypto.randomUUID()}}),
  cancel: (id: string) => request.post<Run>({url: `/v1/runs/${id}/cancel`}),
  upload: (file: File, kind = 'files', mode = '', options: Record<string,string> = {}) => {const data = new FormData(); data.append('file', file); if (mode) data.append('mode',mode); for(const [key,value] of Object.entries(options))data.append(key,value); return request.post<any>({url: `/v1/${kind}`, data, timeout: 120000})},
  ...
  blob: async (id: string) => {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/v1/files/${id}`, {headers: {Authorization: `Bearer ${useUserStore().accessToken}`}})
