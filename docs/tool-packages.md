...
文件字段使用 `format: "tooldeck-file"`。API 提交文件 ID；Worker 在运行前将其替换为容器内只读文件路径。输出文件写入 `TOOLDECK_OUTPUT_DIR`，平台在程序结束后收集并返回 `artifacts`，不接受程序提供的任意宿主机路径。
...
网页上传提供第三方访问、异步结果通知（站内及邮箱）、允许API调用三个选项。开启通知强制后台执行；未验证邮箱的历史账号仅有站内通知。私有工具仅本人和管理员可见，公开工具按站点设置审核后对登录用户可见，不可引用平台密钥。工具详情的API标签包含PHP、Java、Python、Go示例；关闭API的工具不展示此标签，服务端也拒绝API调用。
...
所有配置值加密保存且不回显；按上传者、工具name、提供方式、配置者隔离。不同版本共用同一范围的值；未声明env的新版本继承最近版本定义，显式空数组不继承。两个提供方式的配置独立，切换不会把原凭证公开给使用者。必填未配置返回422。运行时注入声明的变量，日志遮盖原值；工具代码不得主动在stdout、输出文件中泄漏凭证。保留的运行环境变量不能覆盖。
