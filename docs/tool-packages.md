...
ZIP 仍限制64MB、展开128MB、2000条目。构建日志最多保留 1 MB；构建期间约每 2 秒增量持久化，网页每 3 秒刷新，完成后保存最终日志。它不是逐行 SSE，页面刷新或服务重启后仍可查看已经持久化的部分。

同一作者使用相同 `name` 上传的包视为同一工具的不同版本。“我的工具”保留所有版本分别构建、审核和下架；工具广场按“作者 + `name`”合并，只展示最新上传且已构建成功、已审核通过、未下架的版本。不同作者的同名工具不会互相覆盖。新版本构建或审核期间继续展示旧版；新版本通过后自动切换，下架后自动回退到上一可用版本。
...
`execution.max_concurrency` 可选，范围为 0–64，0 表示不额外限制。同作者、同名称的工具版本共享并发计数，使用各版本中最严格的非零限制；全局和单用户限制仍同时生效。对第三方配额敏感或具有非幂等副作用的工具可设置为 1。更多部署配置见 [运行与治理](operations.md)。
